...
echo >> Sources/Swift${Mod}.swift ""
grep '^open class' Sources/${Module}.swift | cut -d' ' -f3 | cut -d: -f1 | sort -u | sed -e 's/^\(.*\)/public typealias _GModule_\1 = \1/' >> Sources/Swift${Mod}.swift
echo >> Sources/Swift${Mod}.swift ""
echo >> Sources/Swift${Mod}.swift "public extension ${Mod} {"
grep 'public protocol' Sources/${Module}.swift | cut -d' ' -f3 | cut -d: -f1 | sort -u | sed -e 's/^\(.*\)/    typealias \1 = _GModule_\1/' >> Sources/Swift${Mod}.swift
echo >> Sources/Swift${Mod}.swift ""
grep '^open class' Sources/${Module}.swift | cut -d' ' -f3 | cut -d: -f1 | sort -u | sed -e 's/^\(.*\)/    typealias \1 = _GModule_\1/' >> Sources/Swift${Mod}.swift
grep '^public typealias' Sources/${Module}.swift | sed 's/^public /    /' >> Sources/Swift${Mod}.swift
echo >> Sources/Swift${Mod}.swift "}"
